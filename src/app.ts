...
import type { Snippet, FilterOptions, Language } from "./types.js";
...
const iconGrid = document.getElementById("icon-grid") as HTMLElement;
const iconList = document.getElementById("icon-list") as HTMLElement;
...
const fLang = document.getElementById("f-lang") as HTMLSelectElement;
const fDesc = document.getElementById("f-desc") as HTMLInputElement;
const fCode = document.getElementById("f-code") as HTMLTextAreaElement;
...
function getLangClass(lang: Language): string {
...
// ── FILTER ─────────────────────────────────────────────────
function applyFilters(all: Snippet[], options: FilterOptions): Snippet[] {
  return all.filter((s) => {
    const matchLang =
      options.language === "all" || s.language === options.language;
...
// ── CARD ───────────────────────────────────────────────────
function createCard(snippet: Snippet): string {
...
function saveSnippet(): void {
  ...
  const newSnippet: Snippet = {
    ...
    code,
    language: fLang.value as Language,
    tags: fTags.value
      .split(",")
      .map((t) => t.trim())
      .filter(Boolean),
...
// ── COPY ───────────────────────────────────────────────────
function copySnippet(id: string, btn: HTMLButtonElement): void {
...
searchInput.addEventListener("input", () => {
  filters = { ...filters, searchQuery: searchInput.value };
  render();
});
...
document.querySelectorAll<HTMLButtonElement>(".nav-item").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    filters = {
      ...filters,
      language: btn.dataset["filter"] as FilterOptions["language"],
    };
    render();
  });
});
